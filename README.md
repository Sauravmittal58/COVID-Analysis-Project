...
All figures below are in [`results/figures/`](results/figures) and are regenerated automatically when `scripts/covid_analysis.R` is run.

### Distribution of the dependent variable

![Distribution and Q-Q plot of Covid_Deaths](results/figures/fig1_distribution_qqplot.png)

`Covid_Deaths` is right-skewed rather than normally distributed, and the Q-Q plot shows clear departure from the reference line in both tails — consistent with the Shapiro-Wilk test result (p ≈ 0).

### Correlation matrix

![Correlation heatmap](results/figures/fig2_correlation_heatmap.png)

The full correlation matrix across all predictors. Strong positive clustering is visible among the health variables and among some travel-mode variables, which is part of the motivation for using PCA to address multicollinearity.

### Top correlates of COVID-19 death rate

![Top 10 correlations bar chart](results/figures/fig3_top_correlations.png)

The 10 variables with the strongest linear association (positive or negative) with `Covid_Deaths`.

### Relationship with key predictors

![Scatterplots of key predictors](results/figures/fig4_scatterplots_key_predictors.png)

Scatterplots (with fitted linear trend) for six of the variables that appear in the best-fit model, showing the direction of each relationship.

### PCA scree plot

![PCA scree plot](results/figures/fig5_pca_scree_plot.png)

Variance explained by each principal component, with the cumulative variance line. The number of components needed to reach ~70% cumulative variance depends on the exact PCA implementation/rotation used (this repo's R workflow uses `psych::principal()` with varimax rotation, reporting 4 components; a standard unrotated PCA on the same standardised data gives a similar but not identical count, shown here for illustration).

### PCA component loadings

![PCA loadings heatmap](results/figures/fig6_pca_loadings_heatmap.png)

How strongly each original variable loads onto the retained principal components — used to interpret what each component broadly represents (e.g. age structure, ethnicity, travel mode).

### Best-fit model: coefficients and residuals

![Model coefficients and residuals](results/figures/fig7_model_coefficients_residuals.png)

Left: estimated coefficients from the best-fit stepwise model. Right: residuals vs fitted values, used to check for non-linearity or heteroscedasticity in the final model.
