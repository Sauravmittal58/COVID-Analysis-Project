...
dist_plot <- ggplot(data, aes(x = .data[[dependent_var]])) +
  ...
  ylab("Density")
print(dist_plot)
ggsave(file.path(figures_dir, "fig1a_distribution.png"), dist_plot, width = 6, height = 4.5, dpi = 150)

# Q-Q plot for normality check
png(file.path(figures_dir, "fig1b_qqplot.png"), width = 600, height = 450)
...
png(file.path(figures_dir, "fig_scatterplots_grid.png"), width = 1200, height = 800)
par(mfrow = c(2, 3))
...
png(file.path(figures_dir, "fig2_correlation_heatmap.png"), width = 900, height = 800)
...
# Bar chart of top 10 correlated variables
top10_cor <- sorted_cor[2:11]
bar_plot <- ggplot(data.frame(var = factor(names(top10_cor), levels = rev(names(top10_cor))),
                               r = as.numeric(top10_cor)),
                    aes(x = var, y = r, fill = r > 0)) +
  geom_col(color = "black") +
  coord_flip() +
  scale_fill_manual(values = c("TRUE" = "#4393C3", "FALSE" = "#D6604D"), guide = "none") +
  ggtitle("Top 10 Variables Correlated with Covid_Deaths") +
  xlab(NULL) + ylab("Pearson correlation (r)")
print(bar_plot)
ggsave(file.path(figures_dir, "fig3_top_correlations.png"), bar_plot, width = 7, height = 5.5, dpi = 150)
...
png(file.path(figures_dir, "fig5_pca_scree_plot.png"), width = 700, height = 500)
...
png(file.path(figures_dir, "fig6_pca_loadings_diagram.png"), width = 800, height = 900)
...
png(file.path(figures_dir, "fig7_regression_diagnostics.png"), width = 900, height = 800)
par(mfrow = c(2, 2))
plot(stepwise_model)
par(mfrow = c(1, 1))
dev.off()
...
# Coefficient plot for the best-fit model (excludes the intercept)
...
coef_plot <- ggplot(coef_df, aes(x = term, y = Estimate, fill = Estimate > 0)) +
  geom_col(color = "black") +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) +
  coord_flip() +
  scale_fill_manual(values = c("TRUE" = "#4393C3", "FALSE" = "#D6604D"), guide = "none") +
  ggtitle("Best-Fit Model: Coefficient Estimates (95% CI)") +
  xlab(NULL) + ylab("Coefficient estimate")
print(coef_plot)
ggsave(file.path(figures_dir, "fig8_model_coefficients.png"), coef_plot, width = 7, height = 6, dpi = 150)
